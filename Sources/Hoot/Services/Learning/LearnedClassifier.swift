...
struct LearnedClassifier: Codable {
    ...
    var learnedFolders: [(folder: String, examples: Int)] {
        folderCounts.sorted { $0.value > $1.value }.map { (folder: $0.key, examples: $0.value) }
    }

    // MARK: - Training

    static func train(on samples: [Sample]) -> LearnedClassifier {
    ...
    func predict(_ features: [String]) -> Prediction? {
    ...
    static func load(from url: URL? = nil) -> LearnedClassifier? {
    ...
    static func crossValidate(_ samples: [Sample]) -> (accuracy: Double, answered: Int, total: Int) {
