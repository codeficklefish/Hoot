...
struct CorrectionLog: Codable {
    ...
    static let trainingWeight = 3

    private static let maximumCorrections = 500

    var isEmpty: Bool { corrections.isEmpty }
    var count: Int { corrections.count }

    mutating func record(features: [String], chose: String, insteadOf rejected: String) {
    ...
    var trainingSamples: [LearnedClassifier.Sample] {
    ...
    static func load(from url: URL? = nil) -> CorrectionLog {
