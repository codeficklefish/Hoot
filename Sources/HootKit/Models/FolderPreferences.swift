...
public struct FolderPreferences: Codable, Equatable {
    ...
    public var entries: [(proposed: String, preferred: String)] {
        mappings
            .sorted { $0.key < $1.key }
    ...
    public func preferredName(for proposed: String) -> String {
        mappings[proposed.lowercased()] ?? proposed
    }

    /// Records that `proposed` should have been `preferred`.
    public mutating func remember(proposed: String, preferred: String) {
    ...
    public mutating func forget(proposed: String) {
    ...
    public static func load(from defaults: UserDefaults = .standard) -> FolderPreferences {
